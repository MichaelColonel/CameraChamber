...
void Camera2::processDataCounts(bool fullChipCalibration, bool splitData,
  IntegratorType integType, ProfileRepresentationType profileType)
{
  Q_D(Camera2);
  Q_UNUSED(fullChipCalibration);
  AbstractCamera::processDataCounts(false, splitData, integType, profileType);
...
//  const std::vector< double >& horizProfStrips = this->getHorizontalProfileStripsNumbers();
  ...
  if (horizProfile)
  {
    ...
    const double& step1 = STRIP_STEP_MM;
    for (Int_t i = 0; i < xBins; ++i)
    {
      horizProfile->SetPoint(i, xBinsBorders[i] + step1 - halfSizeHoriz, Double_t(horizProfData[i]));
    }
  }
}
...
std::vector< double > Camera2::GenerateHorizontalProfileStripsBinsBorders(size_t n)
{
  ...
  v[0] = 0.0;
  const double& step2 = STRIP_STEP_PER_SIDE_MM;
  const double& step1 = STRIP_STEP_MM;
